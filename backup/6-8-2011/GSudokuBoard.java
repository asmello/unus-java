...
public class GSudokuBoard extends JPanel {
	...
	public void paintComponent (Graphics g) {
		...
		for (int y = 0; y < boardSize; ++y)
			for (int x = 0; x < boardSize; ++x) {
				...
				else if (x == wPosX && y == wPosY) g.setColor(Color.lightGray);
						...
						while (next != NULL) {
							if (i == next)
								g.drawString(JLimitedTextField.getChar(i) + "",
									(int) (posX + (double)( (i-min) % subSize)
									* boardWidth / (1.33 * boardSize * subSize)
									+ boardWidth / (2.0 * boardSize * subSize)),
									(int) (posY + (double)( (i-min) / subSize)
									* boardHeight / (1.25 * boardSize * subSize)
									+ boardHeight / (0.96 * boardSize * subSize)));
							next = pValues[y][x].getNext();
						}
					}
	...
	private void setWorkPos(int x, int y) {
		wPosX = x; wPosY = y;
	}
	...
	private void setBoard() {
		timer.stop();
		bigNumStyle = new Font(Font.MONOSPACED, Font.BOLD, 468/boardSize);
		smallNumStyle = new Font(Font.MONOSPACED, Font.PLAIN, 144/boardSize);

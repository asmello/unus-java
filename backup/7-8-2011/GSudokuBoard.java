...
public class GSudokuBoard extends JPanel {
	...
	public void updateSize(Dimension d) {
		boardWidth = (int) (d.width*0.9);
		boardHeight = d.height-120;
	...
	public void paintComponent (Graphics g) {
		...
		for (int y = 0; y < boardSize; ++y)
			for (int x = 0; x < boardSize; ++x) {
				int posX = x * boardWidth/boardSize + boardWidth/18;
		...
		for (int i = 1; i < boardSize; ++i) {
			g.fillRect(i * boardWidth/boardSize + boardWidth/18 - 1,
					2, 2, boardHeight);
			g.fillRect(boardWidth/18 - 1, i * boardHeight/boardSize,
					boardWidth, 2);
		}
		g.setColor(Color.BLACK);
		for (int i = 0; i <= boardSize; i += subSize) {
			g.fillRect(boardWidth/18 - 2, i * boardHeight / boardSize,
					boardWidth, 3);
			g.fillRect(i * boardWidth / boardSize + boardWidth/18 - 2,
					0, 3, boardHeight + 3);
		}
	...
	public int readFile (File f) {
		...
		for (int y = 0; input.hasNextLine(); ++y) {
			...
			if (line.length() != boardSize) return line.length();
			
			for (int x = 0; x < boardSize; ++x) {
				int value =  JLimitedTextField.getValue(line.charAt(x), min,
														boardSize + min);
				if (value != NULL) {
					insert(x, y, value, true);
					fixPos[y][x] = true;
					setConflicts(x, y);
				}
			}
